/// fork from Metamask https://github.com/MetaMask/metamask-extension/blob/develop/app/scripts/controllers/transactions/index.js
...
export default class TransactionController extends EventEmitter {
  ...
  _getCurrentNetworkEIP1559Compatibility?: any;
  _getCurrentAccountEIP1559Compatibility?: any;
  preferencesStore?: any;
  provider?: any;
  getPermittedAccounts?: any;
  blockTracker?: any;
  signEthTx?: any;
  inProcessOfSigning?: any;
  _trackMetaMetricsEvent?: any;
  _getParticipateInMetrics?: any;
  _getEIP1559GasFeeEstimates?: any;
  memStore?: ObservableStore<{
    currentNetworkTxList?: any[];
    unapprovedTxs?: Record<string, any>;
  }>;
  query?: any;
  txGasUtil?: any;
  txStateManager?: any;
  ...
  getUnapprovedTxCount?: any;
  getPendingTxCount?: any;
  getTransactions?: any;
  _getEth?: any;
  ...
  getChainId() {
    ...
    if (networkState.network === 'loading' || Number.isNaN(integerChainId)) {
      return 0;
    }
   ...
   */
  async getCommonConfiguration(fromAddress: string) {
    ...
    if (CommonSupportNetwork.includes(type)) {
      return new Common({
        chain: type,
        hardfork,
      });
  ...
  */
  ...
  async addUnapprovedTransaction(txParams, origin) {
    // validate
    const normalizedTxParams: any = txUtils.normalizeTxParams(txParams);
   ...
   */
  async addTxGasDefaults(txMeta, getCodeResponse?: any) {
   ...
   * @param originalTxId - the id of the txMeta that you want to attempt to cancel
   * @param customGasSettings - overrides to use for gas
   ...
   * each instead.
   * @param originalTxId - the id of the txMeta that you want to speed up
   * @param customGasSettings - overrides to use for gas
   *  params instead of allowing this method to generate them
   */
  ...
  */
   ...
   * @param txId - The tx's ID
   */
  async confirmTransaction(txId: string, txReceipt, baseFeePerGas) {
    ...
    try {
      ...
      const metricsParams: any = { gas_used: gasUsed };
  ...
  */
  ...
  /**
    ...
    @param txId - the tx's Id
    @param txHash - the hash for the txMeta
  */
  ...
  _mapMethods() {
    ...
    this.getSelectedAddress = () => preferenceService.getSelectedAddress();
  ...
  */
  ...
  /**
    ...
    @param txId - the txId of the transaction that has been confirmed in a block
  */
  _markNonceDuplicatesDropped(txId: string) {
  ...
  async getERC20Balance(token, accountAddress) {
    const contract = this._getEth().contract(abi).at(token.address);
    const usersToken = await contract.balanceOf(accountAddress);
    if (!usersToken) {
      return '0x0';
    }
