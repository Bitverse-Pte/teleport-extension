...
import { isValidHexAddress } from '../../../utils/hexstring-utils';
import {
...
const normalizers = {
  from: addHexPrefix,
  to: (to: string, lowerCase: boolean) =>
...
function withoutDigits(hexstr: string): string {
...
export function normalizeAndValidateTxParams(
  txParams: TxParams,
  lowerCase = true
) {
 ...
 */
export function normalizeTxParams(
  txParams: TxParams,
  lowerCase = true
): TxParams {
  ...
  return normalizedTxParams as TxParams;
}
...
function ensureMutuallyExclusiveFieldsNotProvided(
  txParams: TxParams,
  fieldBeingValidated: string,
  mutuallyExclusiveField: string
) {
 ...
 */
function ensureFieldIsString(txParams: TxParams, field: string): void {
...
function ensureProperTransactionEnvelopeTypeProvided(
  txParams: TxParams,
  field: 'gasPrice' | 'maxFeePerGas' | 'maxPriorityFeePerGas'
) {
  switch (field) {
    case 'maxFeePerGas':
 ...
 */
export function validateFrom(txParams: TxParams) {
 ...
 */
export function validateRecipient(txParams: TxParams) {
