/// fork from Metamask https://github.com/MetaMask/metamask-extension/blob/develop/app/scripts/controllers/transactions/tx-gas-utils.js
...
type PendingTransactionTrackerConstructorParams = {
  provider: any;
  nonceTracker: NonceTracker;
  publishTransaction: (rawTx: string) => Promise<any>;
  getPendingTransactions: () => any[];
  approveTransaction: (txId: string) => Promise<any>;
  getCompletedTransactions: (address: string) => any[];
  query?: any;
...
export default class PendingTransactionTracker extends EventEmitter {
  ...
  publishTransaction: (rawTx: string) => Promise<any>;
  approveTransaction: (txId: string) => Promise<any>;
  confirmTransaction: ((...args: any[]) => Promise<any>) | undefined;

  constructor(config: PendingTransactionTrackerConstructorParams) {
    super();
  ...
  */
  ...
  async _resubmitTx(
    ...
    if (!('rawTx' in txMeta)) {
      return this.approveTransaction(String(txMeta.id));
    }
   ...
   * @private
   */
  async _checkPendingTx(txMeta: Transaction): Promise<void> {
   ...
   * @private
   */
  async _checkIfTxWasDropped(txMeta: Transaction): Promise<boolean> {
    const {
      hash: txHash,
      txParams: { nonce, from },
    } = txMeta;
   ...
   * @private
   */
  async _checkIfNonceIsTaken(txMeta: Transaction): Promise<boolean> {
