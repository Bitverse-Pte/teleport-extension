/// fork from Metamask https://github.com/MetaMask/metamask-extension/blob/develop/app/scripts/controllers/transactions/tx-state-manager.js
...
// import { TRANSACTION_STATUSES, TransactionMeta, TxParams } from 'constants/transaction';
import { METAMASK_CONTROLLER_EVENTS } from './lib/events';
import {
...
// type ExcludeSearchableKeys = null | undefined | boolean | any[]
// type SearchableKeys = Exclude<, ExcludeSearchableKeys>
type SearchableKeys = keyof TxParams | keyof Transaction;
type FilterPredicate = ((v: unknown) => boolean | unknown) | any;
...
export default class TransactionStateManager extends EventEmitter {
   ...
   */
  generateTxMeta(opts: Partial<Transaction> = {}): Transaction {
    ...
    let dappSuggestedGasFees: any = null;
   ...
   *  by id
   */
  getUnapprovedTxList(): Record<string, Transaction> {
    const chainId = this.getCurrentChainId();
   ...
   */
  getApprovedTransactions(address: string): Transaction[] {
   ...
   */
  addTransaction(txMeta: Transaction): Transaction {
    ...
    this._deleteTransactions(txsToDelete.map((id) => String(id)));
   ...
   */
  updateTransaction(txMeta: Transaction, note: string) {
    // normalize and validate txParams if present
    if (txMeta.txParams) {
      txMeta.txParams = normalizeAndValidateTxParams(txMeta.txParams, false);
    }
   ...
   * @param [opts.initialList] - If provided the filtering
   *  will occur on the provided list. By default this will be the full list
   *  from state sorted by time ASC.
   * @param [opts.filterToCurrentNetwork=true] - Filter transaction
   *  list to only those that occurred on the current chain or network.
   *  Defaults to true.
   * @param [opts.limit] - limit the number of transactions returned
   *  to N unique nonces.
   * @returns The TransactionMeta objects that all provided
  ...
  getTransactions({
    ...
    searchCriteria?: Partial<Record<SearchableKeys, FilterPredicate>>;
    initialList?: Transaction[];
   ...
   *
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusRejected(txId: string) {
   ...
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusUnapproved(txId: string) {
   ...
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusApproved(txId: string) {
   ...
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusSubmitted(txId: string) {
   ...
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusConfirmed(txId: string) {
   ...
   * @param {string} txId - the target TransactionMeta's Id
   */
  setTxStatusDropped(txId: string) {
   ...
   */
  setTxStatusFailed(txId: string, err: Error) {
   ...
   *  to remove
   */
  wipeTransactions(address: string) {
    // network only tx
    const { transactions } = this.store.getState();
    const network = this.getNetwork();
    const chainId = this.getCurrentChainId();
   ...
   * events are received.
   * @param {string} txId - the TransactionMeta Id
   ...
   */
  _setTransactionStatus(txId: string, status: TransactionStatuses) {
    const txMeta = this.getTransaction(txId);
   ...
   */
  _addTransactionsToState(transactions: Transaction[]) {
    this.store.updateState({
      transactions: transactions.reduce((result: any, newTx) => {
        if (!result) result = {};
   ...
   *
   *
   * @param targetTransactionId - the transaction to delete
   */
  private _deleteTransaction(targetTransactionId: string) {
    const { transactions } = this.store.getState();
   ...
   *
   * @param targetTransactionIds - the transactions to delete
   */
  private _deleteTransactions(targetTransactionIds: string[]) {
    const { transactions } = this.store.getState();
